...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    # The index names are already set up so we need to name the varaible names and tie it to the index
    for person_key, _ in people_dict.items():
        name = _ [NAME_INDEX]
        birth_year = _ [BIRTH_YEAR_INDEX]
        death_year = _ [DEATH_YEAR_INDEX]
        
        # calculated the time/age of death
...
def count_genders(people_dict):
    ...
    male_count = 0
    female_count = 0    
    #We pulled it up in another window and touble shot with the debugger
    # Yeah I am not really to sure what to do at this point. Sorry :(
    for people_key in people_dict.items():
        if  people_key[1][1] == 'M':
            male_count += 1
        else:
            female_count += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key in marriages_dict.items():
        husband_key = marriage_key[1][0]
        wife_key = marriage_key[1][1]
        wedding_year = marriage_key[1][2]
        husband_name = people_dict[husband_key][NAME_INDEX]
        husband_wedding_age = (wedding_year - people_dict[husband_key][BIRTH_YEAR_INDEX])
        wife_name = people_dict[wife_key][NAME_INDEX]
        wife_wedding_age = (wedding_year - people_dict[wife_key][BIRTH_YEAR_INDEX])
